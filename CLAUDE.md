...
- Nothing is pushed without a logged AUDIT PASS from the research-auditor agent, recorded in notes/audit_log.md. The pre push hook in .githooks/pre-push enforces it. A batch is pushed once the auditor returns PASS. Manisha spot checks ten rows of every pushed batch against the rendered PDF pages before the next batch starts; anything she finds is corrected with a decisions entry and re-audited.
