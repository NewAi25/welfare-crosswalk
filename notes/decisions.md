...
The rewritten auditor's first run returned FAIL. Corrected: Stygar classify every locomotion score and active behaviour validation under lower performance, not high, and the two evidence notes now say so; the appendix has no row named Lely activity, so ref 42 is no longer attached to a product on I008 and the note explains why; the IceTag versus CowScout Leg reading of ref 27 is stated precisely; the I007 note now rests on EFSA Table 27's records readily available rather than an unsourced statement about milk buyers. Two rule gaps in definitions.md were closed as version 0.1.1: the hardware class is none when no sensor source names a technology even where EFSA scores by procedure, and an unsure row takes the class of its cheapest commercial candidate.

Scope: the pre push hook comment, two corpus README headings and three products.csv notes still spoke as the predecessor and were edited; products.csv is therefore no longer byte identical to the copied file, and data/README.md now documents its columns here instead of pointing at the predecessor. stats.py no longer prints a count above an arbitrary score of 8; it prints the score distribution.
